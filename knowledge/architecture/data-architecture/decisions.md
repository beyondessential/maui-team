...
Indicators derived from Tamanu data live as dbt models in `tamanu-source-dbt/models/metrics/`, computed in the Tamanu replica DB. One registry row per indicator; physical models may group multiple indicators when they share upstream computation (see below). All consumers (Tamanu aggregation reports, Tupaia via Data Tables (D9), ad-hoc analyses) source from `metric__` - never recompute the underlying logic.

**Metric grouping models — permitted when upstreams are shared.** Each indicator gets its own `metric_definitions.csv` row (registry-level "one per indicator"), but the physical `metric__` model that emits it may be a grouped view covering several `metric_id`s. The grouped view carries `metric_id` as a discriminator column and emits one row per `(metric_id, period_start, …disaggregations…)` slice. This is the canonical pattern when one-model-per-indicator would cause a shared `int__` / `der__` / `can__` upstream to be re-executed multiple times per query — production bundles ship as views (D9, production-promotion), so the replica-side `table` / `incremental` materialisation that would normally amortise repeated upstream work is unavailable. Grouping along disaggregation shape (e.g. one view for `dhis_ncd_category × age × sex × facility` indicators, one for `age × sex × facility` indicators) lets the production query plan execute the shared upstream once per shape rather than once per indicator. Consumers join the grouped view to the registry on `metric_id` to read the definition.
